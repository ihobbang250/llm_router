...
# ────────────── Load FinQA Dataset ──────────────
def load_finqa_dataset(file_path: str, num_samples: int = None, seed: int = 42, random_sample: bool = True):
    """
    Load FinQA dataset from CSV file
    ...
    """
    print(f"Loading FinQA dataset from {file_path}...")
...
def process_sample(
    ...
    Args:
        ...
        reference_field: Field name for reference (optional)
    ...
    try:
        ...
        if reference_field:
            ...
            reference = ""
            # If no reference field, use question directly as prompt without system template
            prompt = question
            if not prompt:
                return None
        ...
        result_entry = {
            "id": sample.get("id", sample.get("_id", i)),  # Use id from dataset, fallback to _id or index
            "question": question,
            "reference": reference,
            "response": response,
        ...
        # Add text (original question) if available
        if "text" in sample:
            result_entry["text"] = sample["text"]
...
def main():
    ...
    parser.add_argument("--dataset-path", type=str, default="flare_finqa_train.csv",
                       help="Path to FinQA dataset CSV")
    ...
    try:
        run_benchmark(
            ...
            question_field="query",  # FinQA CSV uses 'query' field
            reference_field=None,   # No separate reference field, query contains full prompt
